...
class Graphic:
    ...
    def createPlotlyFigures():
        ...
        for ordner in ordner_liste:
            ...
            fig_position = px.line(df_position, x='time_[s]', y='position_[mm]',
                                   title='Position: (' + txt_name_motor + '_' + txt_name_getriebe + '__' + txt_time + ')')
            fig_position.show()

            verzeichnis_current = os.path.join(verzeichnis, 'current.csv')
            df_current = pd.read_csv(verzeichnis_current)
            # Linienplot erstellen
            fig_current = px.line(df_current, x='time_[s]', y='current_[mA]',
                                  title='Current: (' + txt_name_motor + '_' + txt_name_getriebe + '__' + txt_time + ')')
            fig_current.show()
